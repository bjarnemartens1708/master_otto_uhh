...
(() => {
  ...
  const stylesheet =
    ...
    // Skeleton: shown while the sidecar hydrates a persisted slot, or while a
    // src/dropped image is still decoding — replaces the empty "drop" prompt
    // flash and the bare gray frame during load.
    '.skel{position:absolute;inset:0;display:none;background:rgba(0,0,0,.06);overflow:hidden}' +
    '.skel::after{content:"";position:absolute;inset:0;transform:translateX(-100%);' +
    '  background:linear-gradient(90deg,transparent,rgba(255,255,255,.6),transparent);' +
    '  animation:om-slot-shimmer 1.2s infinite}' +
    '@keyframes om-slot-shimmer{100%{transform:translateX(100%)}}' +
    '@media (prefers-reduced-motion:reduce){.skel::after{animation:none}}' +
    ':host([data-skeleton]) .skel{display:block}' +
    ':host([data-skeleton]) .ring{display:none}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '  <div class="skel" part="skeleton"></div>' +
      ...
      this._img.addEventListener('load', () => {
        this._loadPending = false;
        this.removeAttribute('data-skeleton');
    ...
    _render() {
      ...
      if (url && !attrError) {
        ...
        if (prev !== url) {
          ...
          // First fill (no prior image): shimmer while it decodes instead
          // of a bare gray frame. Replacements already show the spinner.
          if (!prev && !this._hidShowing) this.setAttribute('data-skeleton', '');
          this._img.src = url;
          this._ghost.src = url;
        } else {
          ...
          if (this._img.complete) this.removeAttribute('data-skeleton');
          this._releaseMask();
        }
        this._hidShowing = false;
        ...
        // Before the sidecar has hydrated, a persisted slot (has an id, no
        // author src) can't know yet whether it will fill — show the
        // skeleton instead of flashing the empty "drop" prompt.
        const hydrating = !loaded && !!this.id && !attrError;
        this.toggleAttribute('data-skeleton', hydrating);
        this._empty.style.display = (attrError || hydrating) ? 'none' : 'flex';
